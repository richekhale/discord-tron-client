...
async def generate_image(payload, websocket):
    ...
    try:
        ...
        result = await pipeline_runner.generate_image(prompt, model_id, resolution, negative_prompt, steps, positive_prompt, user_config, discord_msg, websocket)
        logging.info("Image generated successfully!")
        discord_msg = DiscordMessage(websocket=websocket, context=payload["discord_first_message"], module_command="send", message="I hope you love boobs!", image=result)
        await websocket.send(discord_msg.to_json())
    except Exception as e:
        import traceback
        logging.error(f"Error generating image: {e}")
        discord_msg = DiscordMessage(websocket=websocket, context=payload["discord_context"], module_command="delete_errors")
        send_result = await websocket.send(websocket)
