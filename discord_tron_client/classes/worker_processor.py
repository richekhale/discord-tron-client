    ...
    async def process_command(self, payload: Dict, websocket: websocket) -> None:
        try:
            ...
            if handler is None:
                ...
                logging.error(
                    f"No handler found in module "
                    + str(payload["module_name"])
                    + " for command "
                    + payload["module_command"]
                    + ", payload: "
                    + str(payload)
                ...
                return
            logging.info(
                f"Running handler for command: {payload['module_command']} in module: {payload['module_name']}"
            )
            logging.debug(
                "Executing incoming "
                + str(handler)
            ...
            logging.error(
                f"Error processing command: {e}, traceback: {traceback.format_exc()} "
            )
            ...
            if "job_id" in payload and payload["job_id"] != "":
                ...
                discord_msg = JobQueueMessage(
                    websocket=websocket,
                    job_id=payload["job_id"],
                    worker_id=identifier,
                    module_command="finish",
                )
