...
class BarkRunner:
    ...
    def generate(self, prompt, user_config):
        ...
        audio, self.sample_rate, semantics = self.driver.generate_long(
            prompt, user_config
        )
    ...
    async def generate_handler(self, payload, websocket):
        ...
        try:
            ...
            logging.debug(
                f"Received result from TTS engine: {output_audio}, {self.sample_rate}"
            )
            ...
            output_audio = base64.b64encode(sound.export(format="mp3").read()).decode(
                "utf-8"
            )
            ...
            discord_msg = DiscordMessage(
                ...
                + "`"
                + prompt[:32]
                + f'...`\nVoice: `{user_config.get("tts_voice")}` Usage stats: {usage}',
                audio_url=url_list,
            ...
            logging.error(
                f"Received an error in BarkRunner.generate_handler: {e}, traceback: {clean_traceback(traceback.format_exc())}"
            )
            discord_msg = DiscordMessage(
                websocket=websocket,
                context=payload["discord_first_message"],
                module_command="edit",
                message=f"We pooped the bed when generating your audio! {e}",
            )
