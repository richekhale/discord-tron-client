...
class BarkRunner:
    ...
    async def generate_handler(self, payload, websocket):
        ...
        try:
            ...
            discord_msg = DiscordMessage(websocket=websocket, context=payload["discord_first_message"], module_command="edit", message=f"We pooped the bed when generating your audio! {e}, traceback: {clean_traceback(traceback.format_exc())})")
            websocket = AppConfig.get_websocket()
            await websocket.send(discord_msg.to_json())
            raise e