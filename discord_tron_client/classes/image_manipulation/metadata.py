...
class ImageMetadata:
    ...
    def encode(image: Image, user_config: dict, attributes: dict = None):
        ...
        metadata.add_text(
            "parameters",
            ImageMetadata.automatic1111_metadata(user_config_copy, attributes),
        )
    ...
    def automatic1111_metadata(user_config: dict, attributes: dict):
        ...
        a1111_output = attributes.get(
            "prompt", user_config.get("positive_prompt", "no prompt given")
        )
        try:
            a1111_output = f"{a1111_output}\n"
            a1111_output = f'{a1111_output}Steps: {user_config.get("steps", 0)},'
            a1111_output = (
                f'{a1111_output} Sampler: {attributes.get("sampler", "DDIM")},'
            )
