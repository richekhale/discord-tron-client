...
prompt_styles = {
    "base": ["{prompt}", "{prompt} striated lines, canvas pattern, blurry"],
    "alec-baldwin": [
...
class PromptManipulation:
    ...
    def should_enable(self, pipeline, user_config: dict = None):
        if (type(pipeline).__name__ == "StableDiffusionXLPipeline") or (
            type(pipeline).__name__ == "StableDiffusionPipeline"
        ):
            return True
        return False
    ...
    def is_valid_pipeline(self, pipeline):
        if not hasattr(pipeline, "tokenizer") and not hasattr(pipeline, "tokenizer_2"):
            raise Exception(
    ...
    def process_long_prompt(self, positive_prompt: str, negative_prompt: str):
        ...
        if self.has_dual_text_encoders(self.pipeline):
            logging.debug(
                f"Running dual encoder Compel pipeline for batch size {batch_size}."
            )
    ...
    def stylize_prompt(user_prompt: str, user_negative: str, user_style: str = None):
        ...
        user_prompt, user_style = PromptManipulation.get_override_style(
            user_prompt, user_style
        )
