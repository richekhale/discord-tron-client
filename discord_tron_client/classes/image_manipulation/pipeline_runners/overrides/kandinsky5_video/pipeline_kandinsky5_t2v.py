...
class Kandinsky5T2VPipeline(DiffusionPipeline, KandinskyLoraLoaderMixin):
    ...
    def fast_sta_nabla(T: int, H: int, W: int, wT: int = 3, wH: int = 3, wW: int = 3, device="cuda") -> torch.Tensor:
        ...
        """
        lat = torch.Tensor([T, H, W]).amax()  # pragma: no cover
        r = torch.arange(0, lat, 1, dtype=torch.int16, device=device)
    ...
    def prepare_latents(
        ...
        image: Optional[torch.Tensor] = None,  # unused in T2V, accepted for I2V override compatibility
    ) -> torch.Tensor:
        """
        ...
        **kwargs,
        ...
        if prompt_embeds_qwen is None:
            if (
                ...
                raise ValueError("Text encoders/tokenizers must be loaded to encode prompts.")
        ...
        if output_type != "latent":
            if self.vae is None:
                raise ValueError("VAE is not loaded; set output_type='latent' or load the VAE to decode video.")
            ...
            video = video / getattr(self.vae.config, "scaling_factor", 1.0)
