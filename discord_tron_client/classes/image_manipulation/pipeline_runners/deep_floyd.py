...
class DeepFloydPipelineRunner(BasePipelineRunner):
    ...
    def _invoke_stage2(
        ...
        stage2_result = self.stage2(
            ...
            num_inference_steps=max(1, min(50, int(self.parameters.get("steps_2", user_config.get("df_inference_steps_2", 20))))),
            output_type=output_type,
            width=s2_width,
            height=s2_height,
            num_images_per_prompt=1,
            guidance_scale=max(0, min(20, float(self.parameters.get("df_guidance_scale_2", user_config.get("df_guidance_scale_2", 5.7))))),
    ...
    def _invoke_stage1(
        ...
        output = self.stage1(
            ...
            num_inference_steps=max(1, min(100, int(self.parameters.get("steps_1", user_config.get("df_inference_steps_1", 30))))),
            generator=generators,
    ...
    def _embeds(self, prompt: str, negative_prompt: str):
        ...
        if "max_sequence_len" in prompt_parameters:
            self.max_sequence_len = max(77, min(512, int(prompt_parameters["max_sequence_len"])))
