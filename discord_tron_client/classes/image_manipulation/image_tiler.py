...
class ImageTiler:
    def __init__(
        self, pil_image: Image, tile_size=64, overlap=8, processing_function=None
    ):
        ...
        self.processing_function = (
            ...
            else self._default_processing_function
        )
    ...
    def _default_processing_function(self, tile):
        logging.warning(
            f"No processing function provided for ImageTiler! Returning tile as-is."
        )
        return tile
    ...
    def _resize_image(self):
        w, h = self.image.size
        new_w = (
            (w + self.tile_size - self.overlap - 1) // (self.tile_size - self.overlap)
        ) * (self.tile_size - self.overlap) + self.tile_size
        new_h = (
            (h + self.tile_size - self.overlap - 1) // (self.tile_size - self.overlap)
        ) * (self.tile_size - self.overlap) + self.tile_size
    ...
    def _split_image(self):
        ...
        logging.debug(
            f"Splitting image into tiles of size {self.tile_size}x{self.tile_size}..."
        )
        tiles = []
        for y in range(0, h, (self.tile_size - self.overlap) or 1):
            ...
            for x in range(0, w, maxcount):
                ...
                if tile_h % 64 != 0:
                    logging.debug(
                        f"Crop height {tile_h} is not a multiple of 64! Cropping..."
                    )
                    tile_h = (tile_h // 64) * 64
                if tile_w % 64 != 0:
                    logging.debug(
                        f"Crop width {tile_w} is not a multiple of 64! Cropping..."
                    )
                    tile_w = (tile_w // 64) * 64
    ...
    def _stitch_tiles(self, tiles, debug_dir=None):
        ...
        logging.debug(
            f"Stitching {len(tiles)} tiles into image of size {total_w}x{total_h}..."
        )
        ...
        for y in range(0, total_h, self.new_tile_size):
            for x in range(0, total_w, self.new_tile_size):
                ...
                if tile_idx > len(tiles):
                    logging.debug(
                        f"Tile idx {tile_idx} not found in tiles list! exiting loop..."
                    )
                    break
                tile = tiles[tile_idx]
                tile_w, tile_h = tile.size
