...
def generate(
    ...
    if prompt is not None:
        if user_prompt is None:
            user_prompt = (
                "What is the first thing you would like to say to StableVicuna?"
            )
    ...
    logging.info(
        f"Generating with user config: {user_config}, user_prompt: {user_prompt}"
    )
    ...
    logging.debug(
        f"Misc parameters: top_k:{top_k} top_p:{top_p} temperature:{temperature} max_tokens:{max_tokens} repeat_penalty:{repeat_penalty}"
    )
