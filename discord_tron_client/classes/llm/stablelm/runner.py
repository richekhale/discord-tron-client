...
class StableLMRunner:
    ...
    async def predict_handler(self, payload, websocket):
        ...
        try:
            ...
            logging.debug(
                f"StableLMRunner predict_handler received result {loop_return}"
            )
            discord_msg = DiscordMessage(
                websocket=websocket,
                context=payload["discord_first_message"],
            ...
            logging.error(
                f"Received an error in StableLMRunner.predict_handler: {e}, traceback: {clean_traceback(traceback.format_exc())}"
            )
            discord_msg = DiscordMessage(
                websocket=websocket,
                context=payload["discord_first_message"],
